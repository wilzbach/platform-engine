...
async def run_test_case_in_suite(suite: Suite, case: Case, logger):
    ...
    if len(errors) > 0:
        print(
            f"Failed to compile the following story:" f"\n\n{all_lines}",
            file=sys.stderr,
        )
        raise errors[0]
    ...
    app.stories = {story_name: story.result().output()}
    app.story_global_contexts = {story_name: {}}
    app.environment = {}
    ...
    try:
        ...
        print(
            f"Failed to run the following story:" f"\n\n{all_lines}",
            file=sys.stderr,
        )
        raise e
