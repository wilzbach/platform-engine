...
def test_get_containerconfig_name(app):
    ...
    config = ContainerConfig(
        ...
        data={"auths": {"registry_url": {"auth": "base64_string"}}},
    )
...
def test_hash_volume_name(patch, story, line):
    ...
    assert (
        Containers.hash_volume_name(story.app, line, service, name) == expected
    )
...
async def test_expose_service(app, patch, async_mock):
    ...
    Kubernetes.create_ingress.mock.assert_called_with(
        ingress_name, app, e, container_name, hostname=hostname
    )
...
async def test_create_and_start_no_action(story):
    ...
    with pytest.raises(ActionNotFound):
        await Containers.create_and_start(
            story.app, {"command": "foo"}, "alpine", "alpine"
        )
...
async def test_start(
    patch, story, async_mock, missing_required_var, run_command, with_volumes
):
    line = {
    ...
    story.app.services = {
        "alpine": {
            ServiceConstants.config: {
                ...
                "actions": {"echo": {}},
                "volumes": {
                    "db": {"persist": True, "target": "/db"},
                    "tmp": {"persist": False, "target": "/tmp"},
                },
                "environment": {
                    "param_1": {"required": True},
                    "alpine_only": {},
                },
            }
        }
    ...
    story.app.environment = {
        "alpine": {"alpine_only": True, "param_1": "hello_world"},
        "global": "yes",
    }
    ...
    patch.object(
        Containers, "get_container_name", return_value="asyncy-alpine"
    )

    patch.object(
        Database, "get_container_configs", new=async_mock(return_value=[])
    )

    expected_volumes = []
    if with_volumes:
        hash_db = Containers.hash_volume_name(story.app, line, "alpine", "db")
        hash_tmp = Containers.hash_volume_name(
            story.app, line, "alpine", "tmp"
        )
        expected_volumes = [
...
def test_format_command_with_format(
    patch, logger, app, echo_service, echo_line
):
    story = Stories.story(app, logger, "echo.story")
