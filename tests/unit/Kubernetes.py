...
def test_find_all_ports():
    services = {
        "alpine": {"http": {"port": 8080}},
        "alpha": {
            "expose": {"console": {"http": {"port": 1882}}},
            "http": {
                "port": 9092,
                "subscribe": {"port": 9090},
                "unsubscribe": {"port": 9091},
            },
        },
        "nested": {
            "a": {
    ...
    assert Kubernetes.find_all_ports(services["alpha"]) == {
        1882,
        9090,
        9091,
        9092,
...
@mark.asyncio
async def test_create_namespace_if_required_existing(patch, app, async_mock):
    res = MagicMock()
    res.code = 200
...
async def test_create_namespace_if_required(
    patch, app, line, async_mock, create_result
):
    res_check = MagicMock()
    res_check.code = 400
...
async def test_delete_resource(patch, story, async_mock, first_res, resource):
    ...
    patch.object(
        Kubernetes, "make_k8s_call", new=async_mock(side_effect=api_responses)
    )
    ...
    assert Kubernetes.make_k8s_call.mock.mock_calls == [
        mock.call(
            story.app.config,
            story.app.logger,
            f"{prefix}/my_app/{resource}/foo" f"?gracePeriodSeconds=0",
            method="delete",
        ),
        mock.call(
            story.app.config,
...
async def test_make_k8s_call(patch, story, async_mock, method):
    ...
    assert (
        await Kubernetes.make_k8s_call(
            story.app.config, story.app.logger, path, payload, method=method
        )
...
@mark.parametrize(
    "resource", ["persistentvolumeclaims", "deployments", "services", "foo"]
)
@mark.parametrize("res_code", [404, 200, 500])
@mark.asyncio
async def test_does_resource_exist(
    patch, story, resource, async_mock, res_code
):
    resp = MagicMock()
    resp.code = res_code
    patch.object(
        Kubernetes, "make_k8s_call", new=async_mock(return_value=resp)
    )
    ...
    expected_path = (
        Kubernetes._get_api_path_prefix(resource)
        + f"/{story.app.app_id}/{resource}/name"
    )
    Kubernetes.make_k8s_call.mock.assert_called_with(
        story.app.config, story.app.logger, expected_path
    )
...
async def test_list_resource_names(story, patch, async_mock):
    ...
    patch.object(
        Kubernetes, "make_k8s_call", new=async_mock(return_value=mock_res)
    )
...
async def test_create_volume(
    story, patch, async_mock, persist, resource_exists
):
    name = "foo"
    patch.object(
        Kubernetes,
    ...
    patch.object(Kubernetes, "_update_volume_label", new=async_mock())
    ...
    expected_path = (
        f"/api/v1/namespaces/{story.app.app_id}" f"/persistentvolumeclaims"
    )
...
async def test_update_volume_label(story, patch, async_mock):
    ...
    payload = {"metadata": {"labels": {"last_referenced_on": "123"}}}
...
async def test_create_ingress(
    ...
    app.services = {
        expose.service: {
            ServiceConstants.config: {
                forward_key_name: {
                    expose.service_forward_name: {"http": http_conf}
                }
            }
        }
    }
    ...
    expected_payload = {
        ...
        "spec": {
            "tls": [{"hosts": [f"{hostname}." f"{app.config.APP_DOMAIN}"]}],
            "rules": [
                {
    ...
    if k8s_api_returned_2xx:
        await Kubernetes.create_ingress(
            ingress_name, app, expose, container_name, hostname
        )
    else:
        with pytest.raises(K8sError):
            await Kubernetes.create_ingress(
...
async def test_create_deployment(patch, async_mock, story, image_pull_policy):
    ...
    story.app.app_id = "my_app"
    patch.object(
        story.app, "image_pull_policy", return_value=image_pull_policy
    )
    image = "alpine:latest"
    tag = image.split(":")[-1]
    ...
    volumes = [
        Volume(persist=False, name="tmp", mount_path="/tmp"),
    ...
    patch.object(
        Database, "get_service_limits", new=async_mock(return_value=limits)
    )

    expected_payload = {
        "apiVersion": "apps/v1",
        "kind": "Deployment",
        "metadata": {"name": container_name, "namespace": story.app.app_id},
        "spec": {
            "replicas": 1,
    ...
    expected_create_path = (
        f"/apis/apps/v1/namespaces/" f"{story.app.app_id}/deployments"
    )
    ...
    assert Database.get_service_tag_uuids.mock.mock_calls == [
        mock.call(
            story.app.config, [{"service_uuid": service_uuid, "tag": tag}]
        )
    ]
...
async def test_create_service(patch, story, async_mock):
    container_name = "asyncy--alpine-1"
    line = {LineConstants.service: "alpine"}
    ...
    patch.object(
        Kubernetes, "wait_for_port", new=async_mock(return_value=True)
    )
    patch.object(asyncio, "sleep", new=async_mock())
    story.app.app_id = "my_app"
    ...
    await Kubernetes.create_service(
        story.app, line[LineConstants.service], container_name
    )
    Kubernetes.make_k8s_call.mock.assert_called_with(
        story.app.config, story.app.logger, expected_path, expected_payload
...
async def test_check_for_image_errors(patch, app, async_mock):
    ...
    Kubernetes.make_k8s_call.mock.assert_called_with(
        app.config, app.logger, f"{prefix}/{app.app_id}" f"/pods?{qs}"
    )
...
@mark.parametrize(
    "service",
    [
        {"name": "first", "configuration": {}, "liveness_probe": None},
        {
            "name": "second",
            "configuration": {
                "health": {
                    "http": {"method": "get", "path": "/healthz", "port": 8000}
                }
            },
            "liveness_probe": {
                "httpGet": {"path": "/healthz", "port": 8000},
...
def test_get_liveness_probe(app, service):
    app.services = {
        service["name"]: {"configuration": service["configuration"]}
    }
