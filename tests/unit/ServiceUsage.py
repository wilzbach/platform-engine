...
@mark.parametrize(
    "value",
    [
        {"input": "1024", "output": (1024.0, None), "suffix_len": 1},
        {"input": "1024m", "output": (1024.0, "m"), "suffix_len": 1},
        {"input": "1024Ki", "output": (1024, "Ki"), "suffix_len": 2},
    ],
)
def test_split_value(value):
    assert (
        ServiceUsage.split_value(value["input"], value["suffix_len"])
        == value["output"]
    )
...
@mark.parametrize(
    "value",
    [
        {"input": "21Ki", "split": (21.0, "Ki"), "output": 21504.0},
        {"input": "1Mi", "split": (1.0, "Mi"), "output": 1048576.0},
        {"input": "1024", "split": (1024.0, None), "output": 1024.0},
    ],
)
...
@mark.parametrize(
    "value",
    [
        {"input": "4m", "split": (4.0, "m"), "output": 0.004},
        {"input": "1k", "split": (1.0, "k"), "output": 1000},
        {"input": "5", "split": (5.0, None), "output": 5.0},
    ],
)
...
async def test_start_metrics_recorder(patch, async_mock, app, metrics):
    ...
    patch.object(
        ServiceUsage, "get_metrics", new=async_mock(return_value=metrics)
    )
        ...
        },
        {
            ...
            "k8s_response": {
                "items": [
                    {
                        "containers": [
                            {"usage": {"cpu": "4m", "memory": "21560Ki"}}
                        ]
                    },
                    {
                        "containers": [
                            {"usage": {"cpu": "14m", "memory": "37040Ki"}}
                        ]
                    },
                ]
            },
...
async def test_get_metrics(patch, async_mock, app, data):
    ...
    expected_path = (
        Kubernetes._get_api_path_prefix("metrics") + f"/pods?{expected_qs}"
    )
    ...
    # execution
    ret = await ServiceUsage.get_metrics(
        data["service_tag_uuid"], app.config, app.logger
    )
