...
async def test_lexicon_execute_streaming_container(patch, story, async_mock):
    ...
    patch.object(Services, 'get_container', new=async_mock())
    patch.object(Lexicon, 'line_number_or_none')
    ...
    Services.get_container.mock.assert_called_with(story, line)
    story.end_line.assert_called_with(
        line['ln'], output=Services.get_container.mock.return_value,
