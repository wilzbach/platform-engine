...
async def test_lexicon_execute(patch, logger, story, line, async_mock, name):
    ...
    if name is not None:
        story.end_line.assert_called_with(
            line["ln"], output=output, assign={"paths": [name]}
        )
    else:
        story.end_line.assert_called_with(
            line["ln"], output=output, assign=None
        )
...
async def test_lexicon_set(patch, logger, story):
    ...
    story.end_line.assert_called_with(
        line["ln"], assign={"$OBJECT": "path", "paths": ["a", "b"]}, output=1
    )
    story.line.assert_called_with(line["next"])
    assert result == Lexicon.line_number_or_none()
...
async def test_lexicon_set_mutation(patch, logger, story):
    ...
    line = {
        ...
        "args": [
            ...
            {"$OBJECT": "mutation", "mutation": "uppercase", "args": []},
        ],
        "next": "2",
    }
    result = await Lexicon.set(logger, story, line)
...
async def test_if_condition_1(patch, logger, magic):
    tree = {
        "1": {"ln": "1", "method": "if", "parent": None, "enter": "2"},
        "2": {"ln": "2", "parent": "1", "next": None},
    }
...
async def test_if_condition_2(patch, logger, magic):
    tree = {
        "1": {"ln": "1", "method": "if", "enter": "2", "next": "3"},
        "2": {"ln": "2", "parent": "1"},
        "3": {"ln": "3", "next": None, "method": "execute"},
    }
...
async def test_if_condition(patch, logger, magic, case, async_mock):
    tree = {
        ...
        "6": {"ln": "6", "parent": "5"},
        "7": {"ln": "7", "method": "else", "parent": None, "enter": "8"},
        "8": {"ln": "8", "parent": "7"},
    }
...
async def test_break(logger, story, line, patch, valid_usage):
    patch.object(
        Lexicon, "_does_line_have_parent_method", return_value=valid_usage
    )
    if valid_usage:
...
def test_lexicon_unless(logger, story, line, patch):
    ...
    logger.log.assert_called_with(
        "lexicon-unless", line, story.build_combined_context()
    )
...
@mark.parametrize(
    "execute_block_return", [LineSentinels.BREAK, LineSentinels.RETURN, None]
)
@mark.asyncio
async def test_lexicon_for_loop(
    patch, logger, story, line, async_mock, execute_block_return
):
    iterated_over_items = []

    async def execute_block(our_logger, our_story, our_line):
        iterated_over_items.append(
            story.resolve({"$OBJECT": "path", "paths": ["element"]})
        )
    ...
    line.update(
        {
            "args": [{"$OBJECT": "path", "paths": ["elements"]}],
            "output": ["element"],
        }
    )
...
async def test_lexicon_while(story, patch, magic, logger, line):
    ...
    patch.object(
        Lexicon, "line_number_or_none", side_effect=proxy_line_number_or_none
    )
    patch.object(Lexicon, "execute_line", side_effect=proxy_execute_line)
    ...
    Lexicon.execute_line.assert_called_with(logger, story, "2")
...
async def test_lexicon_execute_streaming_container(patch, story, async_mock):
    ...
    patch.object(Services, "get_container")
    patch.object(Lexicon, "line_number_or_none")
    ...
    Services.get_container.assert_called_with(story, line)
    story.end_line.assert_called_with(
        line["ln"],
        output=Services.get_container.return_value,
...
async def test_story_execute_function(patch, logger, story, async_mock):
    ...
    story.context_for_function_call.assert_called_with(
        line, story.function_line_by_name()
    )

    Lexicon.execute_block.mock.assert_called_with(
        logger, story, story.function_line_by_name()
    )


@mark.asyncio
async def test_lexicon_execute_escaping_sentinel(
    patch, app, logger, story, async_mock
):
    """
    ...
    patch.object(
        Lexicon,
        "execute_line",
        new=async_mock(return_value=LineSentinels.RETURN),
    )
...
@mark.asyncio
async def test_lexicon_execute_line_generic(
    patch, logger, story, async_mock, method
):
...
@mark.asyncio
@mark.parametrize(
    "line_4_result", ["5", LineSentinels.RETURN, LineSentinels.BREAK]
)
async def test_lexicon_execute_block(
    patch, logger, story, async_mock, line_4_result
):
    story.tree = {
        "1": {"ln": "1", "next": "2"},
        "2": {
    ...
    patch.object(
        Lexicon,
        "execute_line",
        new=async_mock(side_effect=["4", line_4_result, "6"]),
    )
    ...
    if LineSentinels.is_sentinel(line_4_result):
        ...
        assert [mock.call("3"), mock.call("4")] == story.line.mock_calls
...
@mark.parametrize("args", [[{"$OBJECT": "string", "string": "error"}], []])
@mark.asyncio
...
async def test_return_used_with_args(patch, logger, story):
    tree = {"1": {"ln": "1", "method": "return", "args": [{}]}}
    with pytest.raises(StoryscriptError):
        await Lexicon.ret(logger, story, tree["1"])
