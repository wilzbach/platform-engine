...
@mark.asyncio
async def test_listen_to_releases(
    patch, db, magic, config, logger, async_mock
):
    ...
    patch.object(db.con, "add_listener", new=async_mock(side_effect=OSError))

    await Apps.listen_to_releases(config, logger, loop)
    asyncio.create_task.assert_called_with(
        Apps.release_listener_db_con.close()
    )
...
@mark.asyncio
async def test_supervise_listener(
    patch, db, magic, config, logger, async_mock
):
    loop = magic()
    ...
    with pytest.raises(asyncio.TimeoutError):
        await asyncio.wait_for(
            Apps.start_release_listener(config, logger, loop), timeout=3.2
        )
    ...
    with pytest.raises(asyncio.TimeoutError):
        await asyncio.wait_for(
            Apps.start_release_listener(config, logger, loop), timeout=1.2
        )
...
async def test_init_all(
    app_environment, patch, magic, async_mock, config, logger, db
):
    config.APP_ENVIRONMENT = AppEnvironment[app_environment]
    db()
    apps = [{"uuid": "my_app_uuid"}]
    patch.object(
        Database,
        "get_all_app_uuids_for_deployment",
    ...
    Apps.reload_app.mock.assert_called_with(config, logger, "my_app_uuid")
...
async def test_reload_app(
    ...
    if raise_exc:
        patch.object(
            Apps, "deploy_release", new=async_mock(side_effect=raise_exc())
        )
    else:
        patch.object(Apps, "deploy_release", new=async_mock())
    ...
    Apps.destroy_app.mock.assert_called_with(
        old_app, silent=True, update_db_state=True
    )

    if (
        AppEnvironment[app_environment_db]
    ...
    Apps.deploy_release.mock.assert_called_with(config=config, release=release)

    if raise_exc:
        logger.error.assert_called()
        assert ReportingEvent.from_release.mock_calls[0][1][0] == release
        assert (
            ReportingEvent.from_release.mock_calls[0][1][1]
            == Events.APP_RELOAD_FAILED
        )
        assert isinstance(
...
async def test_deploy_release_many_volumes(patch, async_mock):
    ...
    for i in range(10):
        stories["services"][f"service_{i}"] = {
            ServiceConstants.config: {"volumes": {"a": {}, "b": {}}}
        }

    patch.object(
        Apps, "get_services", new=async_mock(return_value=stories["services"])
    )
    await Apps.deploy_release(
        config={},
        release=Release(
...
async def test_get_services(patch, config, logger, async_mock):
    patch.object(
        ...
        "get_by_slug",
        new=async_mock(
            return_value=("slug_uuid", "slug_pull", {"slug": True})
        ),
    )
    patch.object(
        GraphQLAPI,
        "get_by_alias",
        new=async_mock(
            return_value=("alias_uuid", "alias_pull", {"alias": True})
        ),
    )

    asyncy_yaml = {
        "services": {
            "microservice/slack": {"image": "microservice/slack", "tag": "v1"}
        },
        "expose": [{"service": "naked_service"}],
    }
    stories = {"services": ["microservice/slack", "http", "lastfm"]}
...
@mark.asyncio
async def test_destroy_app_exc(patch, async_mock, magic, silent, update_db):
    app = magic()
