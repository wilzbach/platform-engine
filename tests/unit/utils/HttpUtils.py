...
async def test_fetch_with_retry(patch, logger, async_mock):
    ...
    kwargs = {"foo": "bar"}
    ret = await HttpUtils.fetch_with_retry(
        3, logger, "asyncy.com", client, kwargs
    )

    assert ret == client.fetch.mock.return_value
    client.fetch.mock.assert_called_with(
        "asyncy.com", foo="bar", raise_error=False
    )
...
def test_add_params_to_url():
    ...
    assert (
        HttpUtils.add_params_to_url("asyncy.com", {"1": "2"})
        == "asyncy.com?1=2"
    )
    assert (
