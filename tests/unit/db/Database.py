...
def pool(magic, patch, async_cm_mock, async_mock):
    ...
    patch.object(
        con,
        "executemany",
        new=async_mock(side_effect=lambda query, *args: (query, *args)),
    )
...
async def test_database_connect(
    patch, magic, config, async_mock, async_cm_mock
):
...
@mark.asyncio
async def test_get_container_configs(patch, magic, config, pool, async_mock):
    patch.object(
        pool.con,
        "fetch",
        new=async_mock(
            return_value=[{"name": "n1", "containerconfig": "config"}]
        ),
    )
    ...
    assert ret == [ContainerConfig(name="n1", data="config")]

    pool.con.fetch.mock.assert_called_with(
        expected_query, app.owner_uuid, registry_url
    )


@mark.asyncio
@mark.parametrize("app_environment", ["PRODUCTION", "STAGING", "DEV"])
async def test_get_release_for_deployment(
    patch, config, pool, async_mock, app_environment
):
    app_id = "my_app_id"
    expected_query = """
...
async def test_get_service_tag_uuids(
    patch, config, pool, magic, async_mock, data
):
    expected_query = """
...
async def test_create_service_usage(patch, config, pool, async_mock, data):
    ...
    pool.con.executemany.mock.assert_called_with(
        query, [(s["service_tag_uuid"],) for s in data]
    )
...
async def test_update_service_usage(config, pool, data):
    ...
    pool.con.executemany.mock.assert_any_call(
        query1,
        [
...
@mark.parametrize(
    "limits",
    [
        {"memory_bytes": [-1], "cpu_units": [-1]},
        {"memory_bytes": [1, 2, 3, 4], "cpu_units": [5, 6, 7, 8]},
        {"memory_bytes": [209715000] * 10, "cpu_units": [0] * 10},
        {"memory_bytes": [3e6] * 10, "cpu_units": [0] * 10},
        None,
    ],
)
@mark.asyncio
async def test_get_service_limits(patch, config, pool, async_mock, limits):
    ...
    query = """
            ...
            where service_tag_uuid = $1;
            """
    patch.object(pool.con, "fetchrow", new=async_mock(return_value=limits))
    ...
    if limits is None or -1 in limits["memory_bytes"]:
        assert ret == {"cpu": 0, "memory": 209715000}  # 200Mi
    else:
        assert ret == {
