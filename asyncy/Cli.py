...
class Cli:
    ...
    @click.option('--port', help='The port on which the engine is running',
                  default='32781')
    @click.option('--host', help='The host on which the engine is running',
                  default='localhost')
    ...
    def run(app_id, story, block, start, context, environment, port, host):
        ...
        req = http_proxy_pb2.Request(story_name=story, app_id=app_id,
