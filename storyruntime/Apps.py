...
from .constants.Events import (
    APP_DEPLOYED,
    APP_DEPLOY_FAILED,
    APP_DEPLOY_INITIATED,
    APP_INSTANCE_DESTROYED,
    APP_INSTANCE_DESTROY_ERROR,
    APP_RELOAD_FAILED,
)
...
class Apps:
    ...
    async def deploy_release(cls, config: Config, release: Release):
        ...
        if release.maintenance:
            logger.warn(
                f"Not updating deployment, app put in maintenance"
        ...
        try:
            ...
            services = await cls.get_services(
                config, logger, stories.get("yaml", {}), stories
            )
            ...
            await Database.update_release_state(
                logger, config, app_id, release.version, ReleaseState.DEPLOYED
            )

            logger.info(
                f"Successfully deployed app {app_id}@" f"{release.version}"
            )
    ...
    async def init_all(cls, config: Config, glogger: Logger):
        ...
        asyncio.create_task(cls.start_release_listener(config, glogger, loop))
    ...
    async def get_services(
        cls, config: Config, glogger: Logger, asyncy_yaml, stories: dict
    ):
        services = {}
        ...
        for service in all_services:
            ...
            omg.update({"uuid": uuid, "image": image})

            services[service] = {"tag": tag, "configuration": omg}

        return services

    @classmethod
    async def destroy_app(cls, app: App, silent=False, update_db_state=False):
        ...
        Reporter.capture_evt(
            ReportingEvent.from_release(app.release, APP_INSTANCE_DESTROYED)
        )
    ...
    async def reload_app(cls, config: Config, glogger: Logger, app_id: str):
        ...
        if cls.apps.get(app_id) is not None:
            await cls.destroy_app(
                cls.apps[app_id], silent=True, update_db_state=True
            )
        ...
        try:
            ...
            if not can_deploy:
                glogger.warn(
                    f"Another deployment for app {app_id} is in "
            ...
            if release.state == ReleaseState.FAILED.value:
                glogger.warn(
                    f"Cowardly refusing to deploy app "
            ...
            if release.stories is None:
                glogger.info(
                    f"No story found for deployment for "
                    f"app {app_id}@{release.version}. "
                    f"Halting deployment."
                )
                return
            await asyncio.wait_for(
                cls.deploy_release(config=config, release=release),
                timeout=5 * 60,
            )
            glogger.info(f"Reloaded app {app_id}@{release.version}")
        except BaseException as e:
            glogger.error(f"Failed to reload app {app_id}", exc=e)

            if release is not None:
                re = ReportingEvent.from_release(
                    release, APP_RELOAD_FAILED, exc_info=e
                )
            else:
            ...
            if isinstance(e, asyncio.TimeoutError):
                logger = cls.make_logger_for_app(
                    config, app_id, release.version
                )
                await Database.update_release_state(
                    logger,
                    config,
    ...
    async def listen_to_releases(cls, config: Config, glogger: Logger, loop):
        ...
        try:
            ...
            await con.add_listener(
                ...
                lambda _conn, _pid, _channel, payload: asyncio.run_coroutine_threadsafe(
                    cls.reload_app(config, glogger, payload), loop
                ),
            )
    ...
    async def start_release_listener(cls, config, glogger, loop):
        while True:
            ...
            if (
                not con
                or not con._con
                or ReleaseConstants.table not in con._listeners
            ):
