...
Subscription = namedtuple(
    "Subscription", ["streaming_service", "id", "payload", "event"]
)
...
class App:
    ...
    def __init__(self, app_data: AppData):
        ...
        self.story_global_contexts = {
            story_name: {"app": self.app_context}
    ...
    def add_subscription(
        ...
        self._subscriptions[sub_id] = Subscription(
            streaming_service, sub_id, payload, event
        )
    ...
    async def clear_subscriptions_synapse(self):
        url = (
            f"http://{self.config.ASYNCY_SYNAPSE_HOST}:"
    ...
    async def unsubscribe_all(self):
        for sub_id, sub in self._subscriptions.items():
            ...
            url = (
                f"http://{sub.streaming_service.hostname}"
