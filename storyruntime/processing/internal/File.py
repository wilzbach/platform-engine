...
@Decorators.create_service(
    name="file", command="mkdir", arguments={"path": {"type": "string"}}
)
async def file_mkdir(story, line, resolved_args):
    ...
    try:
        ...
        raise StoryscriptError(
            message=f"Failed to create directory: {e}", story=story, line=line
        )
...
async def file_write(story, line, resolved_args):
    ...
    try:
        ...
        if resolved_args.get("binary", False) and not isinstance(
            content, bytes
        ):
            content = bytes(
                str(content), resolved_args.get("encoding", "utf-8")
            )
        ...
        raise StoryscriptError(
            message=f"Failed to write to file: {e}", story=story, line=line
        )
...
async def file_read(story, line, resolved_args):
    ...
    try:
        ...
        if resolved_args.get("binary", resolved_args.get("raw", False)):
            mode = "rb"
        else:
            mode = "r"
        with open(path, mode) as f:
        ...
        raise StoryscriptError(
            message=f"Failed to read file: {e}", story=story, line=line
        )
...
async def file_list(story, line, resolved_args):
    ...
    try:
        ...
        raise StoryscriptError(
            message=f"Failed to list directory: {e}", story=story, line=line
        )


@Decorators.create_service(
    name="file", command="removeDir", arguments={"path": {"type": "string"}}
)
async def file_remove_dir(story, line, resolved_args):
    ...
    try:
        ...
        raise StoryscriptError(
            message=f"Failed to remove directory: {e}", story=story, line=line
        )


@Decorators.create_service(
    name="file", command="removeFile", arguments={"path": {"type": "string"}}
)
async def file_remove_file(story, line, resolved_args):
    ...
    try:
        ...
        raise StoryscriptError(
            message=f"Failed to remove file: {e}", story=story, line=line
        )
